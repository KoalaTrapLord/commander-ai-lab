...
def _parse_text_decklist(text: str, commander_override: str = None) -> dict:
    ...
    for line in text.strip().split("\n"):
        ...
        if re.match(r"^\[commander\]$", line, re.IGNORECASE):
            section = "commander"
            continue
        if re.match(r"^\[(main|deck|mainboard)\]$", line, re.IGNORECASE):
            section = "main"
            continue
        if re.match(r"^\[(sideboard|side)\]$", line, re.IGNORECASE):
            section = "sideboard"
            continue
...
def build_java_command(
    ...
    # Add deck args dynamically (2-4 decks)
...
async def add_deck_card(deck_id: int, req: AddDeckCardRequest):
    ...
    # If this is the commander, sync the deck-level commander fields
    if req.is_commander:
        conn.execute(
            "UPDATE decks SET commander_name = ?, commander_scryfall_id = ?, updated_at = datetime('now') WHERE id = ?",
            (card_name, req.scryfall_id, deck_id)
        )
        conn.commit()
    else:
