...
const Collection = (() => {
    ...
    let state = {
        ...
        preferences: {
            ...
            sortField: 'name',
            sortDir: 'asc',
            visibleColumns: ['quantity','name','mana_cost','type_line','color_identity','cmc','power_toughness','rarity','set_name','tcg_price','edhrec_rank','salt_score','category','finish'],
        },
    };
        ...
        <div id="collection-page">
            ...
            <div class="coll-topbar">
                <div class="coll-topbar-left">
                    <button class="coll-back-btn" onclick="Collection.goBack()">← Simulator</button>
    ...
    const COLUMNS = [
        ...
        { key: 'oracle_text',   label: 'Oracle',    sortable: false },
    ];
    ...
    function renderTable() {
        ...
        tbody.innerHTML = state.items.map(card => {
            ...
            return `<tr class="coll-row" onclick="Collection.openDrawer(${card.id})" data-id="${card.id}">
                ...
                <td class="coll-td coll-td-mana">${manaCost}</td>
                <td class="coll-td coll-td-type">${escapeHtml(card.typeLine || card.type_line || '')}</td>
                <td class="coll-td coll-td-colors">${renderColorPips(card.colorIdentity || card.color_identity)}</td>
                <td class="coll-td coll-td-cmc">${card.cmc ?? '—'}</td>
                <td class="coll-td coll-td-pt">${pt}</td>
                <td class="coll-td coll-td-rarity">${rarity}</td>
                <td class="coll-td coll-td-set" title="${setName}">${setName}</td>
                <td class="coll-td coll-td-price">${formatPrice(card.tcgPrice || card.tcg_price)}</td>
                <td class="coll-td coll-td-edhrec">${edhrecRank}</td>
                <td class="coll-td coll-td-salt ${saltCls}" title="Salt: ${card.saltScore ?? card.salt_score ?? '?'}">${(card.saltScore ?? card.salt_score) != null ? Number(card.saltScore ?? card.salt_score).toFixed(2) : '—'}</td>
                <td class="coll-td coll-td-gc">${gcIcon}</td>
                <td class="coll-td coll-td-category">${renderCategoryChips(card.category || [], card.id)}</td>
                <td class="coll-td coll-td-finish">${renderFinishBadge(card.finish)}</td>
                <td class="coll-td coll-td-oracle" title="${escapeAttr(card.oracleText || card.oracle_text || '')}">${escapeHtml(oracleTrunc)}</td>
            </tr>`;
        }).join('');
    }
