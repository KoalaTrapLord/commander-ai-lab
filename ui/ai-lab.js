...
const AiLab = (() => {
    ...
    // ── State ──────────────────────────────────────────────
    let isRunning = false;
    let selectedDecks = [null, null, null, null];
    let lastResult = null;
...
..."></textarea>
    ...
    function renderPreconGrid(precons) {
        ...
        gridEl.innerHTML = precons.map(p => {
            ...
            const scryfallFallback = `https://api.scryfall.com/cards/named?fuzzy=${encodeURIComponent(p.commander)}&format=image&version=art_crop`;
            const artUrl = p.imageUrl || scryfallFallback;
        ...
        loadPreconArt(gridEl);
    }

    async function loadPreconArt(gridEl) {
        const imgs = gridEl.querySelectorAll('img.precon-card-art[data-commander]');
        for (const img of imgs) {
            const commander = img.dataset.commander;
            if (!commander) continue;
            try {
                const res = await fetch(`${API_BASE}/card-image/${encodeURIComponent(commander)}`);
                if (res.ok) {
                    const data = await res.json();
                    if (data.art_url) { img.src = data.art_url; img.style.display = ''; }
                }
            } catch (_) { /* keep Scryfall fallback */ }
        }
    }
    ...
    async function startBatch() {
        ...
        if (engine === 'deepseek') {
            // DeepSeek: at least 1 deck required (not all 4)
            ...
            // Java Forge: requires all 4 decks
            if (selectedDecks.some(d => !d)) {
                alert('Select a deck for all 4 seats.');
                return;
            }
    ...
    function simulateOneGame(index, rng) {
        ...
        const winningSeat = rng() < 0.05 ? null : Math.floor(rng() * 4);
        ...
        const playerResults = [0, 1, 2, 3].map(seat => ({
            seatIndex: seat,
    ...
    function computeLocalSummary(games, elapsedMs) {
        const perDeck = [0, 1, 2, 3].map(seat => {
    ...
    const SEAT_COLORS = ['#58a6ff', '#f85149', '#3fb950', '#d29922'];
    const SEAT_COLORS_DIM = ['rgba(88,166,255,0.3)', 'rgba(248,81,73,0.3)', 'rgba(63,185,80,0.3)', 'rgba(210,153,34,0.3)'];
    ...
    function renderHeadToHead(result) {
        ...
        // Compute pairwise: for each pair (A, B), how often did A win when both were in the game (always — 4-player pod)
