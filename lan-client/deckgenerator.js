...
const DeckGenerator = (() => {
    ...
    function renderResults(result) {
        ...
        renderUseInGameButton(result);
    }

    function renderUseInGameButton(result) {
        // Only show when running inside the LAN client iframe
        if (window.parent === window) return;

        var existing = document.getElementById('dg-export-game-btn');
        if (existing) existing.remove();

        var btn = document.createElement('button');
        btn.id = 'dg-export-game-btn';
        btn.className = 'dg-btn dg-btn-preview';
        btn.style.cssText = 'margin:12px auto;display:block;border-color:rgba(91,141,217,0.4);color:rgba(91,141,217,0.9);';
        btn.innerHTML = '<span class="dg-btn-icon">&#x1F3AE;</span> Use in Game';
        btn.addEventListener('click', function() {
            var cards = (result.cards || []).map(function(c) {
                return { name: c.name, quantity: c.count || 1, is_commander: false };
            });
            if (result.commander) {
                cards.unshift({ name: result.commander.name, quantity: 1, is_commander: true });
            }
            window.parent.postMessage({
                type: 'DECK_BUILDER_EXPORT',
                deck: cards,
                deckName: result.commander ? result.commander.name + ' Deck' : 'Generated Deck',
                commander: result.commander ? result.commander.name : '',
            }, '*');
            toast('Deck sent to game setup!', 'success');
        });

        $('dg-results').appendChild(btn);
    }
