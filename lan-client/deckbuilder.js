...
/* ── LAN Client API Base URL ────────────────────────────────
   When served from a different host than lab_api.py, set
   window.LAB_API_BASE before this script loads.             */
const LAB_API_BASE = window.LAB_API_BASE || '';
...
async function apiFetch(path, options = {}) {
    const url = path.startsWith('/api') ? LAB_API_BASE + path : path;
...
const DeckBuilder = {
    ...
    _bindTopBar() {
        ...
        if (qs('#deck-export-game-btn')) {
            qs('#deck-export-game-btn').addEventListener('click', () => this._exportToLanClient());
        }
    ...
    async _doImport() {
        ...
        try {
            ...
            if (mode === 'new') {
                ...
                const res = await fetch(LAB_API_BASE + '/api/decks/import-new', {
                    method: 'POST',
                ...
                const res = await fetch(LAB_API_BASE + '/api/decks/' + this.deckId + '/import', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
    ...
    _exportToLanClient() {
        ...
        const commanders = this.deckCards.filter(c => c.is_commander);
        const deck = this.deckCards.map(c => ({
            name: c.card_name,
            quantity: c.quantity || 1,
            is_commander: !!c.is_commander,
        }));
        window.parent.postMessage({
            type: 'DECK_BUILDER_EXPORT',
            deck: deck,
            deckName: this.deckName || 'Untitled',
            commander: commanders.length > 0 ? commanders[0].card_name : '',
        }, '*');
        ToastManager.show('Deck sent to game setup!', 'success');
    },
    ...
    async _exportToSim() {
        ...
        try {
            ...
            const res = await fetch(LAB_API_BASE + '/api/sim/run-from-deck', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
            ...
            while (!done) {
                ...
                const statusRes = await fetch(LAB_API_BASE + '/api/sim/status?simId=' + simId);
                ...
                if (status.status === 'complete') {
                    ...
                    const resultRes = await fetch(LAB_API_BASE + '/api/sim/result?simId=' + simId);
    ...
    async _exportDckToSim() {
        ...
        try {
            const res = await fetch(LAB_API_BASE + '/api/decks/' + this.deckId + '/export-to-sim', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
            });
            const ct = res.headers.get('content-type') || '';
