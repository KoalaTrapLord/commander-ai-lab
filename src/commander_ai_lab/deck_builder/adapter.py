...
class DeckBuilderAdapter:
    ...
    # -- substitution stub --
    def run_substitution(self, cards, commander: str = "", strategy: str = "", **kw):
        """
        No-op substitution -- Ollama pipeline handles collection internally.
        Returns a SimpleNamespace matching the shape DeckGeneratorV3 returns.
        """
        owned = sum(
            1 for c in cards
            if getattr(c, "owned", False)
            or (isinstance(c, dict) and c.get("owned"))
        )
        return SimpleNamespace(
            cards=cards,
            owned_count=owned,
            substituted_count=0,
            missing_count=len(cards) - owned,
        )
        ...
        try:
            ...
            for card in cards:
                ...
                if not name:
                    continue
                row = conn.execute(
                    "SELECT name, scryfall_id, quantity, tcg_price "
                    "FROM collection_entries WHERE name = ? COLLATE NOCASE LIMIT 1",
                    (name,),
                ).fetchone()
                if row:
                    card["owned"] = True
                    card["owned_qty"] = row[2] if row else 0
                    card["from_collection"] = True
                    card["scryfall_id"] = card.get("scryfall_id") or row[1]
                    card["estimated_price_usd"] = float(row[3]) if row[3] else 0
                else:
                    card["owned"] = False
                    card["owned_qty"] = 0
    ...
    def _fill_basic_lands(
        ...
        if needed <= 0:
            return cards
        ci_to_basic = {"W": "Plains", "U": "Island", "B": "Swamp", "R": "Mountain", "G": "Forest"}
        basics = [ci_to_basic[c] for c in color_identity if c in ci_to_basic] or ["Wastes"]
        for i in range(needed):
            ...
            if existing:
                ...
                cards.append({
                    "name": land_name, "count": 1, "category": "lands",
                    "mana_cost": "", "cmc": 0, "type_line": "Basic Land",
                    "color_identity": [], "owned": False, "owned_qty": 0,
                    "from_collection": False, "estimated_price_usd": 0,
                    "source": "auto-fill",
                })
        return cards
