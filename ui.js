...
function initSetupScreen() {
  ...
  setupEl.innerHTML =
    ...
    '<div class="setup-section">' +
    '<div class="setup-label">Decks (Optional)</div>' +
    '<div id="deck-assignment-grid" style="display:flex;flex-direction:column;gap:6px;"></div>' +
    '<button class="btn btn-cancel" style="margin-top:8px;font-size:11px;" onclick="openPreconBrowser()">Browse Commander Precons</button>' +
    '</div>' +
...
function updateDeckAssignmentGrid(playerCount) {
  const grid = document.getElementById('deck-assignment-grid');
  if (!grid) return;
  const preconList = (typeof PRECON_DATA !== 'undefined') ? PRECON_DATA : (typeof PRECONS !== 'undefined') ? PRECONS : null;
  if (!preconList || preconList.length === 0) {
    grid.innerHTML = '<div style="font-size:11px;color:var(--color-text-faint);">Import deck in the Deck Manager after starting.</div>';
    return;
  }

  // Pick random unique precons for each player slot
  var shuffled = preconList.slice();
  for (var si = shuffled.length - 1; si > 0; si--) {
    var sj = Math.floor(Math.random() * (si + 1));
    var tmp = shuffled[si]; shuffled[si] = shuffled[sj]; shuffled[sj] = tmp;
  }
  var defaults = shuffled.slice(0, playerCount);

  grid.innerHTML = '';
  for (let i = 0; i < playerCount; i++) {
    var defaultId = defaults[i] ? defaults[i].id : '';
    const row = document.createElement('div');
    row.className = 'deck-select-row';
    var optionsHtml = '<option value="">— No deck (empty) —</option>';
    preconList.forEach(function(p) {
      var sel = (p.id === defaultId) ? ' selected' : '';
      optionsHtml += '<option value="' + p.id + '"' + sel + '>' + p.name + (p.commander ? ' (' + p.commander + ')' : '') + '</option>';
    });
    row.innerHTML =
      '<span class="deck-select-label" style="color:' + PLAYER_COLORS[i] + '">P' + (i + 1) + ':</span>' +
      '<select id="deck-sel-' + i + '" class="game-input" style="flex:1;height:24px;font-size:11px;">' +
      optionsHtml +
      '</select>';
